 ...
 * Se calcula al momento de generar el préstamo + se suma el interés acumulado
 */
 ...
 * Se acumula conforme se van pagando las cuotas + se suma el interés acumulado
 */
const calcularUtilidad2 = (cuotasPagadas, montoOriginal, totalInteres, numeroCuotasTotal, interesAcumulado = 0) => {
  ...
  // Ganancia = Total pagado × proporción de interés + interés acumulado
 ...
 * Solo cuenta lo recibido después de recuperar el monto prestado + se suma el interés acumulado
 */
const calcularUtilidad3 = (cuotasPagadas, montoOriginal, interesAcumulado = 0) => {
  const totalPagado = cuotasPagadas.reduce((sum, c) => sum + c.abonado, 0);
  
  // Si aún no se ha recuperado el capital, la utilidad 3 es solo el interés acumulado
  if (totalPagado <= montoOriginal) {
    return interesAcumulado;
  }
  
  // Todo lo que excede el capital es utilidad 3 + interés acumulado
...
// ============================================================================
// COMPONENTE: DIÁLOGO DE PAGO FLEXIBLE (CON INTERÉS PERSONALIZADO)
// ============================================================================
...
const DialogoPagoFlexible = ({ 
  ...
  // NUEVO: Estados para "Pagar Saldo"
  ...
  useEffect(() => {
    if (open && cuotas && cuotas.length > indexInicial) {
      ...
      // Resetear campos de Pagar Saldo
      setDineroPrestado('');
      setAbonoCliente('');
  ...
  const handleTipoPagoChange = (tipo) => {
    ...
    switch (tipo) {
      case 'completo':
        ...
        break;
      case 'interes':
        // NUEVO: El interés es editable - Se deja vacío para que el usuario lo ingrese
        setMontoPago('');
        setDistribucion([]);
        setSaldoCalculado(null);
        break;
      case 'personalizado':
        ...
        break;
      case 'pagar_saldo':
        // NUEVO: Limpiar campos y preparar para pagar saldo
        setMontoPago('');
        setDistribucion([]);
        setSaldoCalculado(null);
        break;
  ...
  // NUEVA FUNCIÓN: Calcular saldo automáticamente
  const calcularSaldoAPagar = () => {
    ...
    // Calcular total a pagar con interés
  ...
  const handleMontoChange = (e) => {
    ...
    // NUEVO: Si es pago de interés, NO calcular distribución (será puro interés)
    if (tipoPago !== 'interes') {
  ...
  const handleConfirmar = () => {
    ...
    if (tipoPago === 'pagar_saldo') {
      ...
      // Pasar el saldo calculado como monto
    ...
    // NUEVO: Si es pago de interés, no validar contra el total pendiente
    if (tipoPago !== 'interes') {
    ...
    // NUEVO: Pasar el tipo de pago para identificar si es interés
  ...
  return (
    <Dialog open={open} onClose={handleCerrar} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers>
        ...
        {/* NUEVO: Formulario de Pagar Saldo */}
        ...
        {/* NUEVO: Alerta especial para pago de interés */}
        {tipoPago === 'interes' && (
          <Alert severity="info" sx={{ mt: 2 }}>
...
// ============================================================================
// NUEVO COMPONENTE: DIÁLOGO CALCULAR Y PAGAR SALDO
// ============================================================================
...
// ============================================================================
// COMPONENTE: PANEL DE UTILIDADES (ACTUALIZADO CON INTERÉS ACUMULADO)
// ============================================================================
...
const UtilidadesPanel = ({ 
  ...
  cuotasPagadas,
  sinCronograma = false,
  interesAcumulado = 0  // NUEVO PARÁMETRO
}) => {
  ...
  useEffect(() => {
    ...
    // ACTUALIZADO: Pasar interesAcumulado a las funciones de cálculo
  ...
  return (
    <Paper elevation={3} sx={{ p: 3, mb: 3, backgroundColor: 'rgba(76, 175, 80, 0.05)' }}>
      ...
      {/* NUEVO: Mostrar interés acumulado si existe */}
      {interesAcumulado > 0 && (
        <Alert severity="success" sx={{ mb: 3 }}>
          <strong>Interés Acumulado:</strong> ${formatMoney(interesAcumulado)}<br />
          Este monto se suma a las 3 utilidades
        </Alert>
      )}
      ...
      <Card variant="outlined" sx={{ mb: 2, backgroundColor: 'primary.50', borderColor: 'primary.main', borderWidth: 2 }}>
        <CardContent>
          ...
          <Typography variant="body2" color="text.secondary" paragraph>
            ...
            {interesAcumulado > 0 && ' Incluye el interés acumulado de los pagos de interés.'}
          </Typography>
          
          <Box sx={{ backgroundColor: 'white', p: 2, borderRadius: 1, mb: 2 }}>
            <Typography variant="caption" color="text.secondary">
              Fórmula: (Total a pagar - Monto prestado){interesAcumulado > 0 && ' + Interés acumulado'}
            </Typography>
            <Typography variant="body2" sx={{ fontFamily: 'monospace', mt: 1 }}>
              (${formatMoney(datosPrestamoOriginal.saldoTotalOriginal)} - ${formatMoney(montoOriginal)})
              {interesAcumulado > 0 && ` + $${formatMoney(interesAcumulado)}`} = ${formatMoney(utilidades.utilidad1)}
            </Typography>
          </Box>
          ...
          <Alert severity="info" sx={{ mt: 2 }}>
            📌 Esta utilidad se registra inmediatamente al crear el préstamo{interesAcumulado > 0 && ' + intereses pagados'}
          </Alert>
        </CardContent>
      </Card>
        ...
        <CardContent>
          ...
          {sinCronograma ? (
            <Alert severity="warning">
              Este préstamo no tiene cronograma de cuotas. La Utilidad 2 {interesAcumulado > 0 ? 'es solo el interés acumulado' : 'no aplica'}.
            </Alert>
          ) : (
            <>
              <Box sx={{ backgroundColor: 'white', p: 2, borderRadius: 1, mb: 2 }}>
                <Grid container spacing={2}>
                  ...
                  {interesAcumulado > 0 && (
                    <Grid item xs={12}>
                      <Typography variant="caption" color="text.secondary">
                        Interés Acumulado Sumado
                      </Typography>
                      <Typography variant="h6" color="warning.main">
                        ${formatMoney(interesAcumulado)}
                      </Typography>
                    </Grid>
                  )}
                </Grid>
              ...
              <Alert severity="success" sx={{ mt: 2 }}>
                📌 La Utilidad 2 es la ganancia obtenida progresivamente por cada cuota pagada{interesAcumulado > 0 && ' + intereses'}
              </Alert>
            </>
          )}
        </CardContent>
      ...
      <Card variant="outlined" sx={{ mb: 2, backgroundColor: 'success.50', borderColor: 'success.main', borderWidth: 2 }}>
        <CardContent>
          ...
          <Typography variant="body2" color="text.secondary" paragraph>
            ...
            {interesAcumulado > 0 && ' Los pagos de interés siempre suman a esta utilidad.'}
          </Typography>

          {sinCronograma ? (
            <Alert severity="warning">
              Este préstamo no tiene cronograma de cuotas. La Utilidad 3 {interesAcumulado > 0 ? 'es solo el interés acumulado' : 'no aplica'}.
            </Alert>
          ) : (
            <>
              <Box sx={{ backgroundColor: 'white', p: 2, borderRadius: 1, mb: 2 }}>
              ...
              {utilidades.porcentajeCapitalRecuperado < 100 ? (
                <Alert severity="warning" sx={{ mt: 2 }}>
                  ⏳ Aún no se ha recuperado el capital completo. La Utilidad 3 {interesAcumulado > 0 ? 'incluye el interés acumulado' : 'comenzará después de recibir'} ${formatMoney(montoOriginal)} en pagos.
                </Alert>
              ) : (
                <Alert severity="success" sx={{ mt: 2 }}>
                  ✅ Capital recuperado completo. Todo lo recibido adicional es ganancia neta (Utilidad 3){interesAcumulado > 0 && ' + interés acumulado'}.
                </Alert>
              )}

              <Alert severity="info" sx={{ mt: 2 }}>
                📌 La Utilidad 3 representa la ganancia neta real, una vez el préstamo ya se pagó completamente en capital{interesAcumulado > 0 && ' + todos los intereses pagados'}
              </Alert>
            </>
          )}
        </CardContent>
                ...
                <TableBody>
                  ...
                  {interesAcumulado > 0 && (
                    <TableRow sx={{ backgroundColor: 'warning.50' }}>
                      <TableCell><strong>Interés Pagado</strong></TableCell>
                      <TableCell align="right"><strong>${formatMoney(interesAcumulado)}</strong></TableCell>
                      <TableCell>
                        <Chip label="Sumado a todas" color="warning" size="small" />
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
...
const ClientesTest = () => {
  ...
  // NUEVO: Estado para interés acumulado
  ...
  // NUEVO: Estado para el diálogo de calcular y pagar saldo
  const [dialogoCalcularPagoAbierto, setDialogoCalcularPagoAbierto] = useState(false);

  // Función: Agregar evento al historial
  ...
  // ACTUALIZADO: Aplicar pago flexible con distribución (con soporte para interés y pagar saldo)
  const handleAplicarPago = (monto, distribucion, tipoPago, datosSaldo = null) => {
    // NUEVO: Si es pago de interés, acumular el monto
    if (tipoPago === 'interes') {
      setInteresAcumulado(prev => prev + monto);
      
      agregarEvento(
        'pago',
        'Pago de Interés Registrado',
        `Pago de interés personalizado: $${formatMoney(monto)}. Se suma a las 3 utilidades.`,
        monto
      );
      
      return; // No afecta cuotas
    }
    ...
    if (tipoPago === 'pagar_saldo') {
      agregarEvento(
        ...
        `Saldo pagado: $${formatMoney(monto)}. Dinero prestado: $${formatMoney(datosSaldo.dineroPrestado)}, Abono: $${formatMoney(datosSaldo.abonoCliente)}, Tasa: ${datosSaldo.porcentajeInteres}%, Plazo: ${datosSaldo.tiempo} meses, Fecha: ${dayjs(datosSaldo.fechaPago).format('DD/MM/YYYY')}`,
        monto
      );
    ...
    // Código existente para pagos normales
  ...
  // NUEVO: Manejar pago calculado
  const handleConfirmarPagoCalculado = (datosCalculo) => {
    // Registrar el pago en el historial
    agregarEvento(
      'pago',
      'Pago de Saldo Calculado',
      `Dinero prestado: $${formatMoney(datosCalculo.dineroPrestado)}, Abono: $${formatMoney(datosCalculo.abonoCliente)}, Saldo pagado: $${formatMoney(datosCalculo.saldoFinal)} (${datosCalculo.porcentaje}% interés, ${datosCalculo.plazo} meses)`,
      datosCalculo.saldoFinal
    );

    // Acumular el saldo pagado al interés acumulado
    // Ya que este es un pago que incluye intereses
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
        ...
        <Box sx={{ mt: 3 }}>
          ...
          {tabActual === 1 && (
            <Box>
              {datosPrestamoOriginal ? (
                <>
                  {/* ACTUALIZADO: Pasar interesAcumulado al panel de utilidades */}
                  <UtilidadesPanel 
                  ...
                  {datosPrestamoOriginal.sinCronograma ? (
                    ...
                    <>
                      ...
                      {/* NUEVO: Botón para calcular y pagar saldo */}
      ...
      {/* NUEVO: Diálogo de Calcular y Pagar Saldo */}
      ...
      <Paper elevation={1} sx={{ p: 2, mt: 3, backgroundColor: 'grey.100' }}>
        ...
        <Typography variant="body2" component="div">
          ...
          <ul>
            ...
            <li><strong>Interés Personalizado:</strong> Ingrese manualmente el interés que se suma a las 3 utilidades</li>
            <li><strong>Pagos Flexibles:</strong> Completo, interés personalizado o monto personalizado</li>
            <li><strong>Calcular y Pagar Saldo:</strong> Nuevo método para calcular automáticamente el saldo total a pagar</li>
          ...
          <ul>
            <li>Botón "Pagar" para registrar pagos (completos, parciales o interés personalizado)</li>
          ...
          <strong>💡 NUEVO: Pago de Interés Personalizado:</strong>
          <ul>
            <li>Ingrese manualmente el monto de interés que desee cobrar</li>
            <li>El interés se suma acumulativamente a las 3 utilidades</li>
            <li>No afecta el cronograma de cuotas</li>
            <li>Se registra en el historial como "Pago de Interés"</li>
          </ul>

          <strong>🧮 NUEVO: Calcular y Pagar Saldo:</strong>
          <ul>
            <li><strong>Dinero Prestado:</strong> Ingrese el monto que se prestó al cliente</li>
            <li><strong>Abono del Cliente:</strong> Ingrese lo que el cliente ya ha pagado</li>
            <li><strong>Porcentaje de Interés:</strong> Defina el interés mensual a aplicar</li>
            <li><strong>Tiempo:</strong> Establezca el plazo del préstamo en meses</li>
            <li><strong>Fecha de Pago:</strong> Seleccione cuándo se realiza el pago</li>
            <li><strong>Cálculo Automático:</strong> El sistema calcula el saldo total a pagar</li>
            <li><strong>Botón "Pagar":</strong> Confirme el pago del saldo calculado</li>
            <li>El interés calculado se suma a las 3 utilidades automáticamente</li>
            <li>Se registra en el historial como "Pago de Saldo Calculado"</li>
            <li>No afecta los otros métodos de pago existentes</li>
          </ul>
        </Typography>
      </Paper>
    </Container>
